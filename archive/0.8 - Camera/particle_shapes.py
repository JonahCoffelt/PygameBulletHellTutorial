...
def draw_square(particle):
    ...
    pos = particle.position - particle.game.camera.position
    # Get the width/height of the particle
    length = particle.size * tile_size * particle.life/particle.max_life
    # Draw the circle to the screen
    pg.draw.rect(particle.game.win, particle.color, (pos.x * tile_size + win_size[0]/2 - length/2, pos.y * tile_size + win_size[1]/2 - length/2, length, length))
...
def draw_circle(particle):
    ...
    # Draw the circle to the screen
    pg.draw.circle(particle.game.win, particle.color, (pos.x * tile_size + win_size[0]/2, pos.y * tile_size + win_size[1]/2), radius)
