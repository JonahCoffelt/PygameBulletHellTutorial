...
class Player:
    ...
    def update(self) -> None:
        """
        Handles player inputs/movement and collisions
        """
    ...
    def draw(self) -> None:
        """
        Draws the player to the center of the screen
        """
        
        tile_size = self.game.map.tile_size
        w, h = tile_size * self.size.x, tile_size * self.size.y
        offset = (self.position - self.game.camera.position) * tile_size
        pg.draw.rect(self.game.win, (255, 0, 0), (self.game.win_size[0]/2 - w/2 + offset.x, self.game.win_size[1]/2 - h/2 + offset.y, w, h))