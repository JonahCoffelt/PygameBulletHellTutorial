...
class Weapon:
    def __init__(self, velocity:float=5, acceleration:float=0, direction_spread:float=.1, velocity_spread:float=.1, reload_time:float=.1, burst:int=1, 
                 size:int=.05, life:float=5, shake:float=.15, shake_magnitude:float=.5) -> None:
        # Various attribs which determine how a bullet will fire
...
class WeaponHandler:
    def __init__(self, game) -> None:
        ...
        self.add('shotgun', velocity=15, acceleration=-3, direction_spread=.5, velocity_spread=1, reload_time=.5, burst=8, size=.1, shake=.3)
        self.add('sniper', velocity=25, direction_spread=.01, reload_time=.8, size=.07, shake=.3)
        self.add('rocket', velocity=0, acceleration=15, direction_spread=.01, reload_time=1.5, size=.25, shake=.3)
        self.add('line', velocity=25, acceleration=0, direction_spread=0, reload_time=0, size=.05, shake=0)
    
    def add(self, name, velocity:float=5, acceleration:float=0, direction_spread:float=.1, velocity_spread:float=.1, reload_time:float=.1, burst:int=1, 
            size:int=.05, life:float=5, shake:float=.15, shake_magnitude:float=.5) -> None:
        """
        ...
        self.weapons[name] = Weapon(velocity, acceleration, direction_spread, velocity_spread, reload_time, burst, size, life, shake, shake_magnitude)
    ...
    def update(self):
        ...
        if self.game.mouse_buttons[0] and self.current_weapon and not self.current_weapon.cooldown:
            ...
            if cam.shake: cam.magnitude = max(cam.magnitude, weapon.shake_magnitude)
            else: cam.magnitude = weapon.shake_magnitude
            cam.shake = weapon.shake